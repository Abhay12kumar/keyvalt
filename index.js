...
const isAzureFunctions = Boolean(process.env.FUNCTIONS_WORKER_RUNTIME);
const routePrefix = isAzureFunctions ? "/api" : "";
...
if (!AZURE_KEYVAULT_URL) {
  ...
  if (!isAzureFunctions) process.exit(1);
}
...
router.get("/credentials", async (req, res) => {
  if (!client) {
    return res.status(500).json({ error: "AZURE_KEYVAULT_URL is not configured" });
  }
